package org.donations;

public class DonationsConfig {
    
    public static final boolean DEBUG = false;

    /**
     * Flattr
     */
    public static final boolean FLATTR_ENABLED = true;
    public static final String FLATTR_PROJECT_URL = "http://code.google.com/p/faster-gps";
    // without http:// !
    public static final String FLATTR_URL = "flattr.com/thing/526077/FasterGPS";

    /**
     * Google
     */
    public static final boolean GOOGLE_ENABLED = true;
    public static final String GOOGLE_PUBKEY = "";
    public static final String[] GOOGLE_CATALOG = new String[] { "fastergps.donation.1",
            "fastergps.donation.2", "fastergps.donation.3", "fastergps.donation.5", "fastergps.donation.8",
            "fastergps.donation.13" };

    /**
     * PayPal
     */
    public static final boolean PAYPAL_ENABLED = true;
    public static final String PAYPAL_USER = "dominik@dominikschuermann.de";
    public static final String PAYPAL_CURRENCY_CODE = "EUR";

    // TODO: get from values
    public static final String PAYPAL_ITEM_NAME = "FasterGPS Donation";

}
